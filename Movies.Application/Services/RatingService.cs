...
namespace Movies.Application.Services
{
    public class RatingService : IRatingService
    {
        ...
        public async Task<bool> RateNovieAsync(Guid movieId, int rating, Guid userId, CancellationToken token = default)
        {
            if(rating is <= 0 or > 5)
            {
        ...
        public Task<bool> DeleteRatingAsync(Guid movieId, Guid userId, CancellationToken token = default) =>
            _ratingRepository.DeleteRatingAsync(movieId, userId, token);

        public Task<IEnumerable<MovieRating>> GetRatingForUserAsync(Guid userId, CancellationToken token = default) =>
            _ratingRepository.GetRatingsForUserAsync(userId, token);
    }
}
